      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {
          ...
          stageGraph.outDegrees.foreach(kv => {
            ...
            // if (historyRefCount.contains(kv._1) && kv._2 > 0 &&
            // 2021年12月3日10:32:36
          ...
          // if (cacheRdd != null) {
          //   if (cacheRdd.getStorageLevel == StorageLevel.NONE) {
          //     logWarning(s"[instrument] info: ccacheRdd's storageLevel is None")
          //   } else {
          //     logWarning(s"[instrument] info: cacheRdd's " +
          //       s"storageLevel is ${cacheRdd.getStorageLevel.toString}")
          //   }
          // }
          ...
          // if (maxRdd != null && maxRdd.getStorageLevel == StorageLevel.NONE) {
          //   logInfo(s"[instrument] info: going to cache rdd ${maxRdd.id}")
          //   maxRdd.dependencies.foreach(parentRdd => {
          //     logInfo(s"[instrument] info: cache rdd ${maxRdd.id}'s " +
          //       s"dependency rdd is ${parentRdd.rdd.id}")
          //   })
          //   cachedSet.add(maxRdd)
          //   maxRdd.cache()
          // }
          // instrument code end
